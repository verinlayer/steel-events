// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#![allow(unused_doc_comments)]
#![no_main]

use alloy_primitives::{address, Address, U256};
use alloy_sol_types::{sol, SolValue};
use risc0_steel::{
    ethereum::{EthEvmInput, ETH_SEPOLIA_CHAIN_SPEC},
    Commitment, Event, SteelVerifier,
};
use risc0_zkvm::guest::env;

risc0_zkvm::guest::entry!(main);

/// Specify the event to query using the [`sol!`] macro.
sol! {
    /// ERC-20 transfer event signature.
    interface IERC20 {
        event Transfer(address indexed from, address indexed to, uint256 value);
    }
}

sol! {
    /// ABI-encodable journal.
    struct Journal {
        Commitment commitment;
        // bytes32 blockHash;
        uint256 value;
    }
}

/// Address of the deployed contract to call the function on (USDT contract on Mainnet).
// const CONTRACT: Address = address!("dAC17F958D2ee523a2206206994597C13D831ec7");
const CONTRACT: Address = address!("fe7087098678f792F8B98d821460B885f68B471e");

fn main() {
    let number_inputs: u64 = env::read();
    // let totalValue: U256 = 0;
    let mut totalValue: U256 = U256::from(0);

    if number_inputs == 1 {
        let input: EthEvmInput = env::read();
        let env = input.into_env().with_chain_spec(&ETH_SEPOLIA_CHAIN_SPEC);
        // let block_hash = latest_env.header().seal();
        
        // Query all `Transfer` events of the USDT contract.
        let event = Event::new::<IERC20::Transfer>(&env);
        let logs = event.address(CONTRACT).query();
        let value = logs.iter().map(|log| log.data.value).sum::<U256>();
        totalValue += value;

        let journal = Journal {
            commitment: env.into_commitment(),
            // blockHash: block_hash,
            value: totalValue,
        };
    }
    else if number_inputs > 1 {
        for i in 0..number_inputs - 1 {
            // This is a placeholder for the actual logic that would be executed.
            // In a real scenario, you would perform some operations here.
            let input: EthEvmInput = env::read();
            let env_pre = input.into_env().with_chain_spec(&ETH_SEPOLIA_CHAIN_SPEC);
            let block_hash = env_pre.header().seal();

            // Query all `Transfer` events of the USDT contract.
            let event = Event::new::<IERC20::Transfer>(&env_pre);
            let logs = event.address(CONTRACT).query();
            let value = logs.iter().map(|log| log.data.value).sum::<U256>();
            totalValue += value;
            
            let input: EthEvmInput = env::read();
            let env_next = input.into_env().with_chain_spec(&ETH_SEPOLIA_CHAIN_SPEC);
            // Verify that the previous EVM state is valid wrt the current EVM state.
            if i <= number_inputs - 2 {
                // We initialize the SteelVerifier with the next state, to check the previous commitment.
                SteelVerifier::new(&env_next).verify(env_pre.commitment());
            }
            // SteelVerifier::new(&env_next).verify(env_pre.commitment());
            // Query all `Transfer` events of the USDT contract.
            let event = Event::new::<IERC20::Transfer>(&env_next);
            let logs = event.address(CONTRACT).query();
            let value = logs.iter().map(|log| log.data.value).sum::<U256>();
            totalValue += value;

            if i == number_inputs - 2 {
                // If this is the last input, we can commit the journal.
                // let block_hash = latest_env.header().seal();
                // This commits the sum of all USDT transfers in the current block into the journal.
                let journal = Journal {
                    commitment: env_next.into_commitment(),
                    // blockHash: block_hash,
                    value: totalValue,
                };

                env::commit_slice(&journal.abi_encode());
            }
        }
    }
}